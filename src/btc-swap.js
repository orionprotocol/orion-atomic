...
function audit (contractScript, partyBPubKey) {
...
async function redeem (contract, toAddress, partyBPair) {
  ...
  // <Orion sig> <Orion pubkey> <client secret> OP_TRUE
...
function getContractChunks (clientPkh, orionPkh, lockTime, secretHash) {
  return [
    ...
    // Verify orion signature is being used to redeem the output
    bitcoin.opcodes.OP_DUP,
    bitcoin.opcodes.OP_HASH160,
    orionPkh !== ANY_CHUNK ? bitcoin.crypto.hash160(orionPkh) : ANY_CHUNK,
...
function atomicSwapContract (clientPkh, orionPkh, lockTime, secretHash) {
    return bitcoin.script.compile(getContractChunks(clientPkh, orionPkh, lockTime, secretHash))
}
